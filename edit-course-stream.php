<?php 
	...
	define ("title", "Edit Course Stream");
	define ("page_name", "Edit Course Stream");
	$msg=null;
?>
<?php
$id = $_GET['cid'];
$course_duration = $conn->query("SELECT * FROM course_stream WHERE id='".$_GET['cid']."'");
...
if(isset($_POST['submit']))	{
		...
		$sql=$conn->prepare("UPDATE course_stream SET name=?, url_key=?, status=?, date_modified=? WHERE id='".$_GET['cid']."'");
    ...
    if($conn->affected_rows==1){
    	...
    	echo "<script> setTimeout(function () { window.location.href = 'edit-course-stream.php?cid=$id'; }, 1000); </script>";
    }
}
?>
<!DOCTYPE html>
