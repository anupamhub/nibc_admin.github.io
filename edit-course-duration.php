<?php 
	...
	define ("title", "Edit Course Duration");
	define ("page_name", "Edit Course Duration");
	$msg=null;
?>
<?php
$id = $_GET['cid'];
$course_duration = $conn->query("SELECT * FROM course_duration WHERE id='".$_GET['cid']."'");
...
if(isset($_POST['submit']))	{
		...
		$sql=$conn->prepare("UPDATE course_duration SET name=?, url_key=?, status=?, date_modified=? WHERE id='".$_GET['cid']."'");
    ...
    if($conn->affected_rows==1){
    	...
    	echo "<script> setTimeout(function () { window.location.href = 'edit-course-duarion.php?cid=$id'; }, 1000); </script>";
    }
}
?>
<!DOCTYPE html>
