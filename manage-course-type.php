<?php 
	...
	define ("title", "Manage Course Type");
	define ("page_name", "Manage Course Type");
?>
<?php 
	$msg = null;

	if(isset($_GET['aid'])){
		if($sql = $conn->prepare("UPDATE course_type SET status='1' WHERE id='".$_GET['aid']."'")){
			$sql->execute();
			$msg = "<div class='alert alert-success'><button type='button' class='close' data-dismiss='alert'>×</button>Status Active</div>";
		}
	}
	if(isset($_GET['inid'])){
		if($sql = $conn->prepare("UPDATE course_type SET status='0' WHERE id='".$_GET['inid']."'")){
			$sql->execute();
			$msg = "<div class='alert alert-success'><button type='button' class='close' data-dismiss='alert'>×</button>Status Inactive</div>";
		}
	}
	if(isset($_GET['did'])){
		if($sql = $conn->prepare("DELETE FROM course_type WHERE id='".$_GET['did']."'")){
			$sql->execute();
				...
				<div class="card-body">
				  <table id="example1" class="table table-bordered table-hover">
					...
					<tbody>
						<?php 
						$ctr =0;
						$course_type = $conn->query("SELECT * FROM course_type");
						while($row = $course_type->fetch_assoc()){
							$ctr++;
						?>
					<tr>
						<td><?php echo $ctr; ?></td>
					  ...
					  <td><?php if($row['status']==1) {echo "<div class='mb-xs mt-xs mr-xs btn btn-sm btn-success mbr-5'>Active</div>";} else{ echo "<div class='mb-xs mt-xs mr-xs btn btn-sm btn-warning mbr-5'>InActive</div>";} ?></td>
						<td>
							<a href="edit-course-type.php?cid=<?php echo $row['id']?>" class="btn btn-sm btn-primary mbr-5">
